...
void ext_modules::generate_data(ofstream& source, unsigned int n) {
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    source.write(reinterpret_cast<char*>(&one_byte), sizeof(uint8_t));
  }
}
...
bool ext_modules::make_demodulate(ifstream& input, ofstream& output,
...
int ext_modules::modulate_demodulate_all_qam(ifstream& input, ofstream& output) {
  ...
  const char* signal_files[] = {"data_complex_4.bin", "data_complex_16.bin", "data_complex_64.bin"};
  const char* result_files[] = {"data_after_QAM_4.bin", "data_after_QAM_16.bin", "data_after_QAM_64.bin"};
  const char* stages[] = {"QAM_4 (QPSK)", "QAM_16", "QAM_64"};
...
void ext_modules::file_to_vector(ifstream& src, vector<uint8_t>& dst) {
  dst.clear();
  uint8_t curr_byte;
  while (src.read(reinterpret_cast<char*>(&curr_byte), sizeof(uint8_t))) {
...
long ext_modules::compare_bits_of_vector(vector<uint8_t>& const a, vector<uint8_t>& const b) {
...
int ext_modules::stat_qam_with_noise(modulator::Qam_modulator* modulator,
                                     demodulator::Qam_demodulator* demodulator, const char* prefix,
                                     double noise_limit) {
  ifstream input;
  ifstream input_2;
  ofstream output;
  ofstream result_file;
  string name_stat_miss = "./stat/stat_misses_" + string(prefix) + ".csv";
  write_text_file(name_stat_miss.c_str(), result_file);
